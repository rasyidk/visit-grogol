...
async function main() {
  ...
  // ── Categories ────────────────────────────────────────────
  const categoryData = [
    { name: 'Alam', icon: 'Mountain', color: '#1B7A3E', description: 'Keindahan alam pedesaan yang asri.' },
    { name: 'Budaya', icon: 'Drama', color: '#B8860B', description: 'Warisan seni dan tradisi leluhur.' },
    { name: 'Kuliner', icon: 'UtensilsCrossed', color: '#C2410C', description: 'Cita rasa khas desa wisata.' },
    { name: 'Penginapan', icon: 'BedDouble', color: '#0F766E', description: 'Akomodasi otentik dan nyaman.' },
    { name: 'Petualangan', icon: 'Compass', color: '#7C3AED', description: 'Aktivitas seru penuh tantangan.' },
  ];
  ...
  const destinasi = [
    {
      ...
      description:
        'Air Terjun Pelangi menawarkan pengalaman visual dan spiritual yang tak terlupakan di tengah hutan lindung desa. Aliran air jernih jatuh dari ketinggian membelah tebing berlumut.',
    ...
    },
    {
      ...
      description:
        'Sawah terasering yang membentang hijau keemasan, sempurna untuk fotografi dan meditasi menyambut matahari terbit.',
    ...
    },
    {
      ...
      description:
        'Danau alami dengan permukaan bening memantulkan langit. Tempat sempurna untuk memancing dan bersantai.',
    ...
    },
    {
      ...
      description:
        'Vila eksklusif berbahan bambu dengan sentuhan modern, dikelilingi rimbunnya hutan bambu yang menenangkan.',
      location: 'Desa Grogol',
      price: 1250000,
    ...
    },
    {
      ...
      description:
        'Joglo autentik yang direstorasi dengan cermat, menghadirkan kemewahan tanpa menghilangkan jiwa tradisional.',
      location: 'Desa Grogol',
      price: 850000,
      priceForeign: 850000,
      openHours: '24 Jam',
    ...
    },
    {
      ...
      description:
        'Eko-lodge yang bertengger di atas kanopi hutan, menawarkan koneksi mendalam dengan alam.',
      location: 'Desa Grogol',
      price: 1800000,
  ...
  // ── Banners ───────────────────────────────────────────────
  const banners = [
    { title: 'Kembali ke Alam & Tradisi', subtitle: 'Rasakan kemewahan dalam kesederhanaan.', ctaLabel: 'Jelajahi Atraksi', position: 1 },
    { title: 'Warisan Kuliner yang Menggugah Selera', subtitle: 'Cita rasa lokal turun-temurun.', ctaLabel: 'Lihat Kuliner', position: 2 },
  ];
  ...
  // ── News ──────────────────────────────────────────────────
  const berita = [
    { title: 'Desa Grogol Raih Penghargaan Desa Wisata Berkelanjutan 2024', category: 'Prestasi', excerpt: 'Pengakuan atas komitmen menjaga kelestarian alam dan budaya.' },
    { title: 'Panduan Lengkap Menikmati Sunrise di Sawah Terasering', category: 'Tips', excerpt: 'Waktu terbaik dan spot foto favorit para wisatawan.' },
    { title: 'Workshop Tenun Serat Alam Kini Terbuka untuk Umum', category: 'Aktivitas', excerpt: 'Belajar langsung dari para pengrajin lokal berpengalaman.' },
  ];
  ...
  // ── Events ────────────────────────────────────────────────
  const events = [
    { title: 'Festival Panen Raya', description: 'Syukuran akbar atas hasil bumi yang melimpah.', startDate: new Date('2024-05-15'), ticketPrice: 0, location: 'Balai Desa Grogol' },
    { title: 'Ritual Bersih Desa', description: 'Momentum penyucian diri dan lingkungan secara kolektif.', startDate: new Date('2024-08-02'), ticketPrice: 50000, location: 'Sungai Desa Grogol' },
    { title: 'Pekan Batik & Tenun', description: 'Eksibisi karya-karya terbaik pengrajin lokal.', startDate: new Date('2024-10-20'), ticketPrice: 25000, location: 'Sanggar Kriya Grogol' },
  ];
  for (const e of events) {
    await prisma.event.upsert({
      where: { slug: slugify(e.title) },
      update: {},
      create: { ...e, slug: slugify(e.title), thumbnail: img(`event-${slugify(e.title)}`), content: `<p>${e.description}</p>` },
    });
  }

  // ── Photo gallery ─────────────────────────────────────────
  const fotos = ['Tari Padi Hijau', 'Tenun Serat Alam', 'Sego Wiwit Tradisional', 'Kopi Rempah', 'Sawah Terasering', 'Danau Cermin'];
  for (let i = 0; i < fotos.length; i++) {
    const existing = await prisma.galeriFoto.findFirst({ where: { title: fotos[i] } });
    if (!existing)
      await prisma.galeriFoto.create({
        data: { title: fotos[i], image: img(`galeri-${slugify(fotos[i])}`), category: i < 2 ? 'Budaya' : i < 4 ? 'Kuliner' : 'Alam', position: i },
      });
  }

  // ── Video gallery ─────────────────────────────────────────
  const videos = [
    { title: 'Pesona Desa Grogol dari Udara', videoUrl: 'https://www.youtube.com/watch?v=aqz-KE-bpKQ' },
    { title: 'Proses Pembuatan Tenun Serat Alam', videoUrl: 'https://www.youtube.com/watch?v=ScMzIvxBSi4' },
  ];
  ...
  // ── Testimonials ──────────────────────────────────────────
  const testimoni = [
    { name: 'Amanda Smith', role: 'Travel Blogger', origin: 'Australia', message: 'Sebuah pengalaman yang menenangkan jiwa. Keramahtamahan warga desa dan udara pegunungan yang segar membuat saya ingin kembali lagi setiap tahun.', rating: 5 },
    { name: 'Irwan Hakim', role: 'Kolektor Seni', origin: 'Jakarta', message: 'Produk UMKM-nya sangat berkualitas tinggi. Saya membawa pulang beberapa kain tenun dan kerajinan gerabah yang kini menjadi hiasan utama.', rating: 5 },
    { name: 'Sari Dewi', role: 'Fotografer', origin: 'Bandung', message: 'Lanskap sawah teraseringnya luar biasa untuk fotografi. Momen sunrise-nya benar-benar magis!', rating: 5 },
  ];
