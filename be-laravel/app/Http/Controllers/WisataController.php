...
class WisataController extends Controller
{
    public function index(Request $request)
    {
        ...
        return response()->json([
            'data' => $wisata->items(),
            'meta' => [
                'page' => $wisata->currentPage(),
                'totalPages' => $wisata->lastPage(),
                'limit' => $wisata->perPage(),
                'total' => $wisata->total(),
                'hasNext' => $wisata->hasMorePages(),
                'hasPrev' => $wisata->currentPage() > 1,
            ]
        ]);
    }
    ...
    public function store(Request $request)
    {
        ...
        return response()->json(['data' => $wisata], 201);
    }

    public function show(string $id)
    {
        $wisata = Wisata::where('id', $id)->orWhere('slug', $id)->firstOrFail();
    ...
    public function update(Request $request, string $id)
    {
        ...
        if ($validated['title'] !== $wisata->title) {
            $slug = Str::slug($validated['title']);
            $count = Wisata::where('slug', 'like', "{$slug}%")->where('id', '!=', $id)->count();
            if ($count > 0) {
                $slug = "{$slug}-{$count}";
            }
            $wisata->slug = $slug;
        ...
        $wisata->title = $validated['title'];
        
        if (array_key_exists('titleEn', $validated) || array_key_exists('title_en', $validated)) {
            $wisata->title_en = $validated['titleEn'] ?? $validated['title_en'] ?? null;
        }

        if (array_key_exists('content', $validated)) {
            $wisata->content = $validated['content'];
        }

        if (array_key_exists('contentEn', $validated) || array_key_exists('content_en', $validated)) {
            $wisata->content_en = $validated['contentEn'] ?? $validated['content_en'] ?? null;
        }

        if (array_key_exists('thumbnail', $validated)) {
            if ($wisata->thumbnail && $wisata->thumbnail !== $validated['thumbnail']) {
                $this->deleteImageFile($wisata->thumbnail);
            }
            $wisata->thumbnail = $validated['thumbnail'];
        }

        if (array_key_exists('images', $validated)) {
            $oldImages = $wisata->images ?? [];
            ...
            $wisata->images = $newImages;
        }

        if (array_key_exists('isActive', $validated) || array_key_exists('is_active', $validated)) {
            $wisata->is_active = $validated['isActive'] ?? $validated['is_active'] ?? true;
        }

        if (array_key_exists('isGalleryActive', $validated) || array_key_exists('is_gallery_active', $validated)) {
            $wisata->is_gallery_active = $validated['isGalleryActive'] ?? $validated['is_gallery_active'] ?? true;
        }
