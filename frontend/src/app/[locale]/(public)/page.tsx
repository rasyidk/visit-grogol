...
import { usePageHero, useTestimoni } from '@/hooks/usePublicData';
import { useTranslations, useLocale } from 'next-intl';

const img = (s: string, w = 800, h = 600) => `https://picsum.photos/seed/${s}/${w}/${h}`;

export default function ProfilPage() {
  const { data: hero } = usePageHero('home');
  ...
  return (
    <>
      ...
      <section className="relative min-h-screen w-full overflow-hidden flex items-center justify-center">
        <Image src={hero.background || '/HeroBanner.jpg'} alt="Desa di balik awan" fill priority className="object-cover object-center" />
          ...
          <Reveal>
            <div className="max-w-4xl mx-auto flex flex-col items-center">
              ...
              <p className="mt-6 max-w-2xl text-lg sm:text-xl text-white/85 font-normal leading-relaxed">
                {hero.description || tHero('subtitle')}
              </p>
