...
import { useBudayaDetail } from '@/hooks/usePublicData';
...
export default function BudayaDetailPage({ params }: { params: { slug: string; locale: string } }) {
  const { data: budaya, isLoading, isError } = useBudayaDetail(params.slug);
  ...
  if (isError || !budaya) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center space-y-4">
        <h1 className="text-3xl font-bold text-ink">Budaya tidak ditemukan</h1>
        <Link href="/budaya" className="btn btn-brand">
  ...
  const displayContent = isEn ? (budaya.contentEn || budaya.content_en || budaya.content) : budaya.content;

  return (
    <article className="min-h-screen pb-20 pt-28">
      {/* ── Header ──────────────────────────────────────── */}
      <header className="container-wide mb-12">
        <Link href="/budaya" className="inline-flex items-center text-sm font-medium text-ink-muted hover:text-brand-600 transition-colors mb-8">
      ...
      {/* ── Photo Gallery ──────────────────────────────── */}
      {(budaya.isGalleryActive ?? budaya.is_gallery_active ?? true) && budaya.images && budaya.images.length > 0 && (
        <GallerySlider images={budaya.images} title={displayTitle} />
      )}
