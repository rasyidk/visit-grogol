...
import { useKulinerDetail } from '@/hooks/usePublicData';
...
export default function KulinerDetailPage({ params }: { params: { slug: string; locale: string } }) {
  const { data: kuliner, isLoading, isError } = useKulinerDetail(params.slug);
  ...
  if (isError || !kuliner) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center space-y-4">
        <h1 className="text-3xl font-bold text-ink">Kuliner tidak ditemukan</h1>
        <Link href="/kuliner" className="btn btn-brand">
          Kembali ke List Kuliner
        </Link>
      </div>
    );
  }
  ...
  const displayContent = isEn ? (kuliner.contentEn || kuliner.content_en || kuliner.content) : kuliner.content;

  return (
    <article className="min-h-screen pb-20 pt-28">
      {/* ── Header ──────────────────────────────────────── */}
      <header className="container-wide mb-12">
        <Link href="/kuliner" className="inline-flex items-center text-sm font-medium text-ink-muted hover:text-brand-600 transition-colors mb-8">
      ...
      {/* ── Photo Gallery ──────────────────────────────── */}
      {(kuliner.isGalleryActive ?? kuliner.is_gallery_active ?? true) && kuliner.images && kuliner.images.length > 0 && (
        <GallerySlider images={kuliner.images} title={displayTitle} />
      )}
