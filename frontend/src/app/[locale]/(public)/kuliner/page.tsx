...
import { useKuliner } from '@/hooks/usePublicData';
...
export default function KulinerPage() {
  ...
  return (
    <>
      <section className="container-wide pt-32 text-center sm:pt-40 mb-12">
        <Reveal>
          <SectionHeading
            align="center"
            title={isEn ? "Culinary Delights" : "Ragam Kuliner"}
            description={isEn 
          ...
          {isLoading ? (
            ...
            <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
              {kulinerList.map((kuliner, i) => {
                ...
                return (
                  <Reveal key={kuliner.id} delay={i * 0.08}>
                    <Link href={`/kuliner/${kuliner.slug}`} className="group block overflow-hidden bg-white rounded-3xl shadow-soft hover:shadow-card transition-shadow">
